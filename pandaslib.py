...
def df_to_excel_append(df: _pd.DataFrame, filename: str,  sheet_name: str, writer_kwargs={}, to_excel_kwargs={}):  # noqa
    """
    Append a DataFrame [df] to existing Excel file [filename]
    into [sheet_name] Sheet.
    If [filename] doesn't exist, then this function will create it.

    It cant do anything fancy like append data to a table or append data after the last found data.

    It can append to am existing sheet, see ExcelWriter args

    Args:
        filename: File path or existing ExcelWriter (Example: '/path/to/file.xlsx')
        df: dataframe to save to workbook
        sheet_name : Name of sheet which will contain DataFrame.
        writer_kwargs: A dictionary of kwargs passed to pd.ExcelWriter
        to_excel_kwargs: A dictionary of kwargs passed to df.to_excel


    Returns: None
    """
    filename = _path.normpath(filename)
    with _pd.ExcelWriter(filename, engine='openpyxl', mode='a', **writer_kwargs) as writer:
