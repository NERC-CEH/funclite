...
import funclite.arraylib as _arraylib
...
def permuted_correlation(list_a, list_b, test_stat, iterations=0, method=EnumMethod.kendall, out_greater_than_test_stat=0):
    ...
    for dummy in range(0, int(iterations)):
        ...
        if method == EnumMethod.kendall:
            teststat, pval = _stats.kendalltau(list_a, permuted)  # noqa
        elif method == EnumMethod.pearson:
            teststat, pval = _stats.pearsonr(list_a, permuted)  # noqa
        elif method == EnumMethod.spearman:
            teststat, pval = _stats.spearmanr(list_a, permuted)  # noqa
        else:
            raise ValueError('Enumeration member not in e_method')

        if teststat > test_stat:
            out_greater_than_test_stat[0] += 1  # noqa
...
def correlation(a, b, method=EnumMethod.kendall):
    ...
    if method == EnumMethod.kendall:
        ...
        teststat, pval = _stats.pearsonr(lst_a, lst_b)  # noqa
    elif method == EnumMethod.spearman:
        teststat, pval = _stats.spearmanr(lst_a, lst_b)  # noqa
    else:
        raise ValueError('Enumeration member not in e_method')
...
def permuted_teststat_check(csvfile, test_col, stat_value_to_check):
    ...
    if stat_value_to_check >= 0:
        res = (df.iloc[0:] > stat_value_to_check).sum()  # noqa
    else:
        res = (df.iloc[0:] < stat_value_to_check).sum()  # noqa
...
def quantile_bin(nd, percentiles=None, zero_as_zero=False):
    ...
    def get_quantile_ranges(nd, percentiles, exclude_zeros=False, use_scipy=True):  # noqa
    ...
    def get_bin_label(x, ranges, zero_as_zero=False):  # noqa
...
def best_fit_distribution(data, bins=200, ax=None):
    ...
    #  _stats.frechet_r, _stats.frechet_l,
    DISTRIBUTIONS = [
        ...
        _stats.foldcauchy, _stats.foldnorm, _stats.genlogistic, _stats.genpareto, _stats.gennorm, _stats.genexpon,
